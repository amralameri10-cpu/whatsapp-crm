import {
  ...
  timestamp, jsonb, index, unique, customType
...
// تخزين دائم للوسائط داخل PostgreSQL. يفصل البيانات الثنائية عن صف الرسالة
// حتى تبقى قوائم المحادثات سريعة، وتُحذف الوسائط تلقائياً مع الرسالة والإنستنس.
...
export const messagesRelations = relations(messages, ({ one }) => ({
  ...
  media: one(messageMedia, { fields: [messages.id], references: [messageMedia.messageId] }),
}));
...
export const contacts = pgTable('contacts', {
  ...
  instancePhoneIdx: index('contacts_instance_phone_idx').on(t.instanceId, t.phone),
}));

export const contactsRelations = relations(contacts, ({ one }) => ({
  team: one(team, { fields: [contacts.teamId], references: [team.id] }),
  instance: one(instances, { fields: [contacts.instanceId], references: [instances.id] }),
