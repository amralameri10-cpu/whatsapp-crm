...
async function setup() {
  ...
  // ترقية آمنة لقواعد البيانات الموجودة قبل إضافة instance_id إلى contacts.
  await sql`ALTER TABLE "contacts" ADD COLUMN IF NOT EXISTS "instance_id" integer;`;

  await sql`
    UPDATE "contacts"
    SET "instance_id" = "chats"."instance_id"
    FROM "chats"
    WHERE "contacts"."chat_id" = "chats"."id"
      AND "contacts"."instance_id" IS NULL;
  `;
  ...
  await sql`
    ...
    BEGIN
      IF NOT EXISTS (
        ...
        WHERE conname = 'contacts_instance_id_instances_id_fk'
      ) THEN
        ALTER TABLE "contacts"
          ADD CONSTRAINT "contacts_instance_id_instances_id_fk"
  ...
  await sql`
    CREATE INDEX IF NOT EXISTS "contacts_instance_phone_idx"
    ON "contacts" ("instance_id", "phone");
  `;
