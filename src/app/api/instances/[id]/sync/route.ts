...
async function upsertContact(
  ...
  if (!existing) {
    await db.insert(contacts).values({ teamId, instanceId, chatId, phone, name });
    return true;
  }
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const contact of contactsResult) {
      ...
      if (phone) {
        if (await upsertContact(ctx.teamId, instance.id, null, phone, name)) syncedContacts++;
      }
    }
    ...
    for (const evoChat of evolutionChats) {
      try {
        ...
        if (!isGroupJid(remoteJid) && phone) {
          if (await upsertContact(ctx.teamId, instance.id, chat.id, phone, name)) syncedContacts++;
        }
