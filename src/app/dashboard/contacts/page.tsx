...
import { fetcher, formatPhoneForDisplay } from '@/lib/utils';
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        {!isLoading && (!data || data.length === 0) && (
          <div className="text-center py-16 text-zinc-400">
            ...
            <p className="text-xs mt-1">ستظهر هنا تلقائياً عند إنشاء جهات اتصال من المحادثات</p>
          </div>
        )}
          ...
          {data?.map((c) => (
            <div key={c.id} className="flex items-center gap-3 py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-zinc-900 dark:text-zinc-50">{c.name || 'بدون اسم'}</p>
