...
export async function signUp(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  // No invite: only allowed if no team exists yet (first setup) → becomes Super Admin
  const existingTeam = await db.select().from(teamTable).limit(1);
  if (existingTeam.length > 0) {
    return { error: 'النظام مُفعّل بالفعل. اطلب دعوة من المسؤول.' };
  }

  const [newUser] = await db.insert(users).values({ email, name, passwordHash, role: 'admin' }).returning();
